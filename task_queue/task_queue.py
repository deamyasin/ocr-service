...
celery_app = Celery(
    'ocr_tasks',
    broker=f'redis://{config.settings.REDIS_HOST}:{config.settings.REDIS_PORT}/0',
    backend=f'redis://{config.settings.REDIS_HOST}:{config.settings.REDIS_PORT}/0'
)
...
def process_file(
    ...
    operation: str,
    language: str = "eng",
    core_count: int = config.settings.DEFAULT_CORE_COUNT
) -> Dict:
